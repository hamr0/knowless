...
| `startLogin` | ({email, nextUrl?, sourceIp?, subjectOverride?, bodyOverride?, bypassRateLimit?}) | Promise\<{handle, submitted: true}\> | Programmatic magic-link send for "use first, claim later" flows. Same 12-step sham-work as form. `subjectOverride` (AF-9) replaces `cfg.subject` per call. `bodyOverride` (AF-26, v0.2.2) is a `({url}) => string` template fn that replaces the default body — knowless still composes the URL and validates the rendered output (ASCII, URL on its own line, ≤2048 chars); `bodyFooter` still appends. `bypassRateLimit: true` (AF-10) opts trusted server-side callers (CLI, cron, worker) out of IP-rate-limit accounting. **`sourceIp` is what per-IP rate limiting buckets on — pass the *real client* IP. Behind a reverse proxy, `req.socket.remoteAddress` is the proxy's address (a constant), which collapses all callers into one bucket and silently disables per-IP limits; use the exported `determineSourceIp(req, auth.config.trustedProxies)` instead. The built-in `login` route already does this for you. **Precondition:** your proxy must set `X-Forwarded-For` to the real peer (nginx `$remote_addr`) — the resolver trusts the leftmost element, so both *appending* (`$proxy_add_x_forwarded_for`) and *omitting* the directive (nginx forwards the client's raw header) leave it client-controllable, re-enabling unlimited per-IP-bucket spoofing (verified end-to-end against real nginx; OPS §7.2).** SPEC §7.3a. AF-7.3. |
...
import {
  ...
  determineSourceIp,   // pure: real client IP from XFF/X-Real-IP when peer is a trusted proxy
} from 'knowless';
```
